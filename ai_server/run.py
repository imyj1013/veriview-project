    ...
    for dir_path in directories:
        if not os.path.exists(dir_path):
            try:
                ...
                print(f"⚠️ 디렉토리 생성 실패: {dir_path} - {str(e)}")
...
def parse_arguments():
    ...
    parser.add_argument('mode', nargs='?', choices=['test', 'main'], 
...
def display_startup_info(mode, d_id_status, aistudios_status, d_id_api_status, aistudios_api_status, d_id_connection_status):
    ...
    print("🎬 D-ID (우선 사용) 상태:")
    print(f"  - 모듈 상태: {'✅' if d_id_available else '❌'} {d_id_msg}")
    print(f"  - API 키: {'✅' if d_id_api_configured else '❌'} {d_id_api_msg}")
    print(f"  - API 연결: {'✅' if d_id_connected else '❌'} {d_id_connection_msg}")
    ...
    print(f"  - 모듈 상태: {'✅' if aistudios_available else '❌'} {aistudios_msg}")
    print(f"  - API 키: {'✅' if aistudios_api_configured else '❌'} {aistudios_api_msg}")
    ...
    if d_id_available and d_id_api_configured and d_id_connected:
        ...
        print("  - 주요 서비스: ⚠️  AIStudios 폴백 사용")
        print("  - 캐싱 시스템: ✅ 활성화")
    else:
    ...
    if not d_id_api_configured and d_id_available:
        print()
        print("⚠️  D-ID API 키가 설정되지 않았습니다!")
...
def load_environment_variables():
    ...
    try:
        ...
        print("⚠️  python-dotenv가 설치되지 않음 - 환경 변수 수동 설정 필요")
        return False
    except Exception as e:
    ...
    if args.mode == "test":
        ...
        try:
            ...
            if d_id_initialized:
                ...
                print("⚠️ D-ID 초기화 실패 - AIStudios 폴백 시도")
                ...
                if aistudios_initialized:
                    ...
                    print("⚠️ AIStudios 폴백도 실패 - 샘플 영상 모드로 동작")
        ...
        try:
            ...
            if not os.path.exists('main_server.py'):
                print("⚠️ main_server.py가 없습니다. 테스트 모드로 대체 실행합니다.")
                print()
                print("🔄 테스트 서버로 대체 실행...")
                from server_runner import app, initialize_analyzers, initialize_d_id, initialize_aistudios
            ...
            try:
                ...
                if d_id_initialized:
                    ...
                    print("⚠️ D-ID 초기화 실패 - 샘플 영상 모드로 동작")
