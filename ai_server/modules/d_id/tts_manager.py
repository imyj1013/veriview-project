...
class TTSManager:
    ...
    def __init__(self):
        ...
        # 면접관용 음성 매핑 (D-ID TTS와 동일)
        self.interviewer_voices = {
            'male': 'ko-KR-InJoonNeural',        # 신뢰감 있는 남성
            'female': 'ko-KR-SunHiNeural'       # 전문적인 여성
        }
        
        # 토론자용 음성 매핑 (안정적인 Azure TTS 음성 사용)
        self.debater_voices = {
            'male': 'ko-KR-InJoonNeural',        # 안정적인 남성 음성
            'female': 'ko-KR-SunHiNeural'       # 전문적인 여성 음성
        }
        
        # D-ID 통합 음성 매핑 (안정성을 위해 기본 음성 사용)
        self.d_id_voice_mapping = {
            'interviewer_male': 'ko-KR-InJoonNeural',
            'interviewer_female': 'ko-KR-SunHiNeural',
            'debater_male': 'ko-KR-InJoonNeural',
            'debater_female': 'ko-KR-SunHiNeural'
        }
    ...
    def optimize_script_for_speech(self, script: str) -> str:
        ...
        # 특수 문자 정리
        optimized = re.sub(r"['\"‘’“”\[\]{}<>()`~@#$%^&*_=+\-\\|;:/]", ' ', optimized)
