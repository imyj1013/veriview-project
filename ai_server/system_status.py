...
def check_system_status():
    ...
    for dir_path, description in directories:
        if os.path.exists(dir_path):
            ...
            print(f"⚠️ {description}: {dir_path}/ (디렉터리 없음)")
...
def check_server_status(url, server_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️ {server_name}: HTTP {response.status_code}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for module, status in modules.items():
                    status_icon = "✅" if status == "사용 가능" else "❌"
                    print(f"   {status_icon} {module}: {status}")
            else:
                print(f"⚠️ AI 서버 테스트 실패: HTTP {response.status_code}")
        except Exception as e:
    ...
    if backend_status:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️ 백엔드 서버 테스트 실패: HTTP {response.status_code}")
        except Exception as e:
    ...
    if ai_status and backend_status:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️ AI-백엔드 연동 테스트 실패: HTTP {response.status_code}")
        except Exception as e:
