...
def check_file_exists(file_path, description):
    ...
    if os.path.exists(file_path):
        print(f"[성공] {description}: {file_path}")
        return True
    else:
        print(f"[실패] {description}: {file_path} (파일 없음)")
        return False
...
def check_system_status():
    ...
    print(f"[위치] 현재 디렉터리: {current_dir}")
    ...
    for dir_path, description in directories:
        if os.path.exists(dir_path):
            print(f"[성공] {description}: {dir_path}/")
        else:
            print(f"[주의] {description}: {dir_path}/ (디렉터리 없음)")
...
def check_dependencies():
    ...
    for package_name, description in required_packages:
        try:
            ...
            print(f"[성공] {description}: {package_name}")
        except ImportError:
            print(f"[실패] {description}: {package_name} (설치 필요)")
            missing_packages.append(package_name)
    
    if missing_packages:
        print(f"\n[주의] 설치가 필요한 패키지: {', '.join(missing_packages)}")
...
def check_server_status(url, server_name):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[성공] {server_name}: 정상 작동 중")
            return True
        else:
            print(f"[주의] {server_name}: HTTP {response.status_code}")
            return False
    except requests.ConnectionError:
        print(f"[실패] {server_name}: 연결 실패 (서버 미실행)")
        return False
    except Exception as e:
        print(f"[실패] {server_name}: 오류 ({str(e)})")
        return False

def check_running_servers():
    """실행 중인 서버 확인"""
    print("\n" + "="*60)
    print("[서버] 서버 실행 상태 확인")
    print("="*60)
...
def run_quick_test():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for module, status in modules.items():
                    status_icon = "[성공]" if status == "사용 가능" else "[실패]"
                    print(f"   {status_icon} {module}: {status}")
            else:
                print(f"[주의] AI 서버 테스트 실패: HTTP {response.status_code}")
        except Exception as e:
            print(f"[실패] AI 서버 테스트 오류: {str(e)}")
    ...
    if backend_status:
        ...
        try:
            ...
            if response.status_code == 200:
                print("[성공] 백엔드 서버 테스트 성공")
            else:
                print(f"[주의] 백엔드 서버 테스트 실패: HTTP {response.status_code}")
        except Exception as e:
            print(f"[실패] 백엔드 서버 테스트 오류: {str(e)}")
    ...
    if ai_status and backend_status:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[성공] AI-백엔드 연동 테스트 성공 ({posting_count}개 공고 추천)")
            else:
                print(f"[주의] AI-백엔드 연동 테스트 실패: HTTP {response.status_code}")
        except Exception as e:
            print(f"[실패] AI-백엔드 연동 테스트 오류: {str(e)}")

def show_api_endpoints():
    """API 엔드포인트 정보"""
    print("\n" + "="*60)
    print("[목록] 주요 API 엔드포인트")
    print("="*60)
    
    print("\n[AI] AI 서버 (http://localhost:5000):")
    ...
    print("\n[백엔드] 백엔드 서버 (http://localhost:4000):")
...
def main():
    ...
    print("\n" + "="*60)
    print("[팁] 추가 도움이 필요하시면 다음 명령어를 실행하세요:")
